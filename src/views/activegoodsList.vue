...
<div class="detail-container">
    <div class="detail-data-box" v-show="!showOrderList && !showEditPage">
        <header class="search-header">
            ...
            <a class="action-btn" style="display: inline-block;" download="导出商品列表.xlsx" :href="downloadUrl">批量导出</a>
                ...
                <tbody>
                    <tr v-for="(row, index) in goodsList" :key="index">
                        ...
                        <td>
                            ...
                            <button class="action-btn" style="padding:0 .3rem;" v-if="row['activegoods_edit'] == 'T'" @click.prevent.stop="editGoods(index)">编辑</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="editor-body hideScrollBar goodsEdit">
            ...
            <div class="editor-groups">
                ...
                <div class="items">
                    ...
                    <div class="item" v-if="goods.type == 1 || goods.type == 2">
                        <label>渠道名称</label>
                        <select v-model="goods.channel_id">
                            <option :value="channel.id" v-for="(channel, idx) in channelList" :key="idx">{{channel.name}}</option>
                        </select>
                    </div>
                    <div class="item" v-if="goods.type == 1">
                        <label>结算形式</label>
                        <select v-model="goods.settlement_type">
                            <option value="1">单笔服务费价格</option>
                            <option value="2">单笔服务费比例</option>
                        </select>
                    </div>
                    <div class="item" v-if="goods.type == 1">
                        <label>服务费</label>
                        <input type="text" v-model="goods.service_fee">
                    </div>
                    <div class="item" v-if="goods.type == 2">
                        <label>推广佣金比例</label>
                        <input type="text" v-model="goods.spread_commission_rate">
                    </div>
                    <div class="item" v-if="goods.type == 3">
                        <label>商品佣金比例</label>
            ...
            <div class="editor-groups">
                ...
                <div class="items">
                    <div class="item">
                        <label>活动时间</label>
                        <flat-pickr class="search-time-picker" v-model="goods.start_time" placeholder="开始时间" :config="dateConfig"></flat-pickr>
                        <div class="split-line"><div></div></div>
                        <flat-pickr class="search-time-picker" v-model="goods.end_time" placeholder="结束时间" :config="dateConfig"></flat-pickr>
                    </div>
                    <div class="item">
                        <label>活动状态</label>
                        <select :value="activeStatus" disabled>
                    ...
                    <div class="item">
                        <label>活动支出</label>
                        <input type="text" v-model="goods.spread_price">
                    </div>
                    <div class="item" style="display: block; display: flex;">
                        <label style="vertical-align: top;">备注</label>
                        <textarea v-model="goods.remark"></textarea>
                    </div>
                </div>
            </div>
            <div class="editor-groups">
...
export default {
    ...
    data(){
        return {
            ...
            'newgoods_name': '',
            'newgoods_platform': '',
            goods: {id: '', 'goods_url': '', type: 1, 'channel_id': '', 'settlement_type': 1, 'service_fee': 0, remark: '', status: '', 'shop_commission_rate': 0, 'spread_commission_rate': 0, 'spread_price': 0, 'is_active': 1},
    ...
    created(){
        this.showLoading()
        Promise.all([
            this.loadFields(setting.urls.appFields, { 'field_type': 'activeGoodsList' }, 'get'),
            this.loadTBData(setting.urls.activeGoodsList, { page: 1 }, 'get'),
    ...
    computed: {
        downloadUrl(){
            return setting.baseUrl + setting.urls.activeGoodsList + '?ids=' + this.checkedArr.join(',') + '&goods_id=' + this.goods_id + '&platform=' + this.platform + '&channel_id=' + this.channel_id + '&status=' + this.status + '&sale_num=' + this.sale_num + '&start_time=' + this.start_time + '&end_time=' + this.end_time + '&is_excel=1&skey=' + this.$cookies.get('skey')
        },
        activeStatus(){
            const now = new Date()
            return now > new Date(this.goods.end_time) ? 3 : now < new Date(this.goods.start_time) ? 1 : 2
        }
    },
    methods: {
        prevClicked(){
        ...
        saveEditedGoods(){
            ...
            if((this.goods.type == 1 || this.goods.type == 2) && !this.goods.channel_id){
                this.alert('渠道必选')
                return
            }
            if(this.goods.type == 1 && !this.goods.settlement_type){
                this.alert('结算方式必选')
                return
            }
            if(this.goods.type == 1 && !this.goods.service_fee && this.goods.service_fee !== 0 && this.goods.service_fee !== '0'){
                this.alert('服务费必填')
                return
            }
            if(this.goods.type == 2 && !this.goods.spread_commission_rate && this.goods.spread_commission_rate !== 0 && this.goods.spread_commission_rate !== '0'){
                this.alert('推广佣金比例必填')
                return
            }
            if(!this.goods.start_time || !this.goods.end_time){
                this.alert('活动起止时间必选')
                return
            }
            // if(!this.goods.spread_price && this.goods.spread_price !== 0 && this.goods.spread_price !== '0') {
            //     this.alert('活动支出费用必填')
            //     return
            // }
            if(!this.goods.remark){
                this.alert('备注必填')
                return
            }
            return request({
                url: setting.urls.activeGoodsAdd,
                method: 'post',
        ...
        editGoods(index){
            ...
            const now = new Date()
            // this.activeStatus = now > this.goods.end_time ? 3 : now < this.goods.start_time ? 1 : 2
            this.showEditPage = true
        },
        createNewChannel(){
            this.editable = true
            this.goods = {id: '', 'goods_url': '', type: 1, 'channel_id': '', 'settlement_type': 1, 'service_fee': 0, remark: '', status: '', 'shop_commission_rate': 0, 'spread_commission_rate': 0, 'spread_price': 0, 'is_active': 1}
        ...
        operateGoods(){
            return request({
                url: setting.urls.activeGoodsOperate,
                method: 'post',
                data: qs.stringify({
        ...
        uploadExcel(){
            ...
            request({
                url: setting.urls.servicegoodsImport,
                method: 'post',
                data:formData,
                headers: {
                ...
                if(rst.status == 200 && rst.data.code == 200){
                    this.alert('导入服务费商品成功')
                    this.loadGoodsList()
                }else{
                    this.alert(rst.data.message || '导入服务费商品失败')
                }
            }).catch(e =>{
                this.alert(e.message || '导入服务费商品失败')
            }).then(() => {
                this.hideLoading()
        ...
        makeData(dt){
            ...
            this.$data['goods_add'] = dt.actions.activegoods_add || 'F'
            this.$data['goods_import'] = dt.actions.activegoods_import || 'F'
        ...
        loadGoodsList(pageNo){
            this.checkedArr = []
            this.allChecked = false
            this.loadTBData(setting.urls.activeGoodsList, {
                page: pageNo || 1,
