<template>
    <div class="detail-container">
        <div class="detail-data-box">
            ...
            <div class="table_title">云瞻信息CPS业务日交易汇总数据统计</div>
...
export default {
    ...
    data: () => {
        ...
        return {
            ...
            'tableHeader': setting.tableHeader.cpsdealYear,
            'tbType': 'common',
            fields: [],
            pageData: {
                'total_page': 0,
    ...
    created: function(){
        ...
        Promise.all([
            ...
            this.loadTBData(setting.urls.cpsdealYear, {'company_id': this.company_id, page: 1, 'statistics_year': this.start_time}, 'get')
        ]).then(rst => {
    ...
    computed: {
        downloadUrl: function(){
            return setting.baseUrl + setting.urls.cpsdealYear + '?company_id=' + this.company_id + '&statistics_year=' + this.start_time + '&page=' + this.pageData.page + '&is_excel=1&skey=' + this.$cookies.get('skey')
        }
    },
    methods: {
        loadData: function(pageNum){
            this.showLoading()
            this.loadTBData(setting.urls.cpsdealYear, {'company_id': this.company_id, page: pageNum || 1, 'statistics_year': this.start_time}, 'get').then(dt => {
