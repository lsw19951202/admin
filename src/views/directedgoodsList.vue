...
<div class="detail-container">
    <div class="detail-data-box" v-show="!showOrderList && !showEditPage">
        <header class="search-header">
            ...
            <a class="action-btn" style="display: inline-block;" download="导出定向商品列表.xlsx" :href="downloadUrl">批量导出</a>
            <!-- 更改批量状态修改 -->
                ...
                <tbody>
                    <tr v-for="(row, index) in goodsList" :key="index">
                        ...
                        <td>
                            ...
                            <button class="action-btn" style="padding:0 .3rem;" v-if="row['directedgoods_edit'] == 'T'" @click.prevent.stop="editGoods(index)">编辑</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="editor-body hideScrollBar goodsEdit">
            ...
            <!-- 新增推广类型 -->
             <div class="editor-groups">
                ...
                <div class="items">
                    <div class="item">
                        <label>推广类型</label>
                        <select v-model="goods.is_direct">
                            <option value="">请选择</option>
                            <option value="1">定向商品</option>
                            <option value="2">非定向商品</option>
                        </select>
                    </div>
                    <!-- 新增新建推广时间 -->
                    <div class="search-group">
            ...
            <div class="editor-groups">
                ...
                <div class="items">
                    ...
                    <div class="item">
                        <label>商家佣金比例</label>
                        <input type="text" v-model="shop_commission_rate" :disabled="goods.is_direct==''|| goods.is_direct==2 ? true : false">
                    </div>
                    <div class="item">
                        <label>推广佣金比例</label>
                        <input type="text" v-model="goods.spread_commission_rate" :disabled="goods.is_direct==2">
                    </div>
                </div>
            </div>
            <div class="editor-groups">
...
export default {
    ...
    data(){
        return {
            ...
            'newgoods_name': '',
            'newgoods_platform': '',
            goods: {id: '', 'goods_url': '', type: 2, 'channel_id': '', 'spread_commission_rate': 0,'start_time': '','end_time': '','is_direct':''},
    ...
    created(){
        this.showLoading()
        Promise.all([
            this.loadFields(setting.urls.appFields, { 'field_type': 'directedGoodsList' }, 'get'),
            this.loadTBData(setting.urls.directGoodsList, { page: 1 }, 'get'),
    ...
    computed: {
        downloadUrl(){
            return setting.baseUrl + setting.urls.directGoodsList + '?ids=' + this.checkedArr.join(',') + '&goods_id=' + this.goods_id + '&platform=' + this.platform + '&channel_id=' + this.channel_id + '&sale_num=' + this.sale_num + '&start_time=' + this.start_time + '&end_time=' + this.end_time + '&is_excel=1&skey=' + this.$cookies.get('skey')
        }
    },
    updated(){
        if(this.goods.is_direct==2){
            this.$data['shop_commission_rate']=this.goods.spread_commission_rate
        }
    },
    methods: {
        prevClicked(){
        ...
        saveEditedGoods(){
            ...
            if(!this.goods.channel_id){
                this.alert('渠道必选')
                return
            }
            if(!this.goods.spread_commission_rate && this.goods.spread_commission_rate !== 0 && this.goods.spread_commission_rate !== '0'){
                this.alert('推广佣金比例必填')
                return
            }
            return request({
                url: setting.urls.directGoodsAdd,
                method: 'post',
        ...
        createNewChannel(){
            this.editable = true
            this.goods = {id: '', 'goods_url': '', type: 2, 'channel_id': '', 'spread_commission_rate': 0}
        ...
        operateGoods(){
            return request({
                url: setting.urls.directGoodsOperate,
                method: 'post',
                data: qs.stringify({
        ...
        uploadExcel(){
            ...
            request({
                url: setting.urls.directedgoodsImport,
                method: 'post',
                data:formData,
                headers: {
                ...
                if(rst.status == 200 && rst.data.code == 200){
                    ...
                    this.alert(rst.data.message || '导入定向商品失败')
                }
            }).catch(e =>{
                this.alert(e.message || '导入定向商品失败')
            }).then(() => {
                this.hideLoading()
        ...
        makeData(dt){
            ...
            this.$data['goods_add'] = dt.actions.directedgoods_add || 'F'
            this.$data['goods_import'] = dt.actions.directedgoods_import || 'F'
        ...
        loadGoodsList(pageNo){
            this.checkedArr = []
            this.allChecked = false
            this.loadTBData(setting.urls.directGoodsList, {
                page: pageNo || 1,
