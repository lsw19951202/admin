    ...
    <div class="detail-data-box">
        <div class="goodsContainer">
            ...
            <div class="ypjGoods" v-show="!showHdkGoods">
                ...
                <!-- 新建商品 -->
                <set-up v-if="showNewTable" :type='type' @callOff='callOffEvent' @preservationEvent='preservationFn' :selectMessge='selectMessge'></set-up>
            </div>
        </div>
    </div>
  </div>
...
export default {
    ...
    methods:{
        ...
        pubHDKGoods(){
            ...
            request({
                url: setting.urls.highHdkGoodsAdd,
                method: 'post',
                data: qs.stringify({
        ...
        getNewFreeHeader(pageNum){//表头请求
            this.showLoading()
            request({
                url:setting.urls.higServantListHeader,
                method:'get',
                params:{}
            }).then(res=>{
                console.log(res,"高佣表头")
                if(res.status == 200){
        ...
        getNewFreeTbody(pageNum){//表格请求
            this.showLoading()
            request({
                url:setting.urls.higServantList,
                method:'get',
                params:{
                    page:pageNum || 1,
        ...
        newly(){//新建按钮
            ...
            this.type = 'highServant'
            this.selectMessge = ''
        },
        operateList(){//发布/下架
            this.showLoading()
            request({
                url:setting.urls.higServanOperate,
                method:'post',
        ...
        uploadExcel: function(){//批量导入
            ...
            request({
                url: setting.urls.higServantImport,
                method: 'post',
                data: formData,
                headers: {
        ...
        preservationFn(e){//新建商品
            ...
            this.showLoading()
            request({
                url:setting.urls.higServantAdd,
                method:'post',
                data:qs.stringify(e),
            }).then(res=>{
                if(res.data.code == 200){
                    console.log(res,"新建高佣99999")
                    this.getNewFreeTbody()
