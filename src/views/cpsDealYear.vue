    ...
    <div class="detail-container hideScrollBar">
        <div class="detail-data-box nopadding">
            <ul class="tab-box">
                ...
                <li :class="'tab-item' + (company_id == 0 ? ' selected' : '')" style="left: 290px;" @click="tabClicked(0)">月度汇总</li>
            </ul>
...
export default {
    ...
    computed: {
        getTitle: function(){
            if(this.company_id){
                return this.company[this.company_id] + 'CPS业务年交易汇总数据统计'
            }else{
                return '云瞻信息CPS业务年交易汇总数据统计'
            }
        },
        downloadUrl: function(){
            return setting.baseUrl + setting.urls.cpsdealYear + '?company_id=' + this.company_id + '&statistics_year=' + (this.company_id == 1 ? this.start_time_sc : (this.company_id == 2 ? this.start_time_cd : this.start_time_all)) + '&page=' + 1 + '&is_excel=1&skey=' + this.$cookies.get('skey')
        },
        getTimeIn: function(){
    ...
    created: function(){
        this.showLoading()
        return Promise.all([
            this.loadFields(setting.urls.cpsFields, {'order_type': 'financedealyear'}),
            this.loadTBData(setting.urls.cpsdealYear, {'company_id': 1, page: 1, 'statistics_year': this.start_time_sc}, 'get'),
            this.loadTBData(setting.urls.cpsdealYear, {'company_id': 2, page: 1, 'statistics_year': this.start_time_cd}, 'get'),
            this.loadTBData(setting.urls.cpsdealYear, {'company_id': 0, page: 1, 'statistics_year': this.start_time_all}, 'get')
        ]).then(rst => {
    ...
    methods: {
        ...
        loadData: function(){
            ...
            this.showLoading()
            this.loadTBData(setting.urls.cpsdealYear, queryParams, 'get')
                .then((rst) => {
