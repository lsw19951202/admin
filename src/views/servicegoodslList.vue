...
<div class="detail-container">
    <div class="detail-data-box" v-show="!showOrderList && !showEditPage">
        <header class="search-header">
            ...
            <a class="action-btn" style="display: inline-block;" download="导出服务费商品列表.xlsx" :href="downloadUrl">批量导出</a>
                ...
                <tbody>
                    <tr v-for="(row, index) in goodsList" :key="index">
                        ...
                        <td>
                            ...
                            <button class="action-btn" style="padding:0 .3rem;" v-if="row['servicegoods_edit'] == 'T'" @click.prevent.stop="editGoods(index)">编辑</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
export default {
    ...
    created(){
        this.showLoading()
        Promise.all([
            this.loadFields(setting.urls.appFields, { 'field_type': 'serviceGoodsList' }, 'get'),
            this.loadTBData(setting.urls.serviceGoodsList, { page: 1 }, 'get'),
    ...
    computed: {
        downloadUrl(){
            return setting.baseUrl + setting.urls.serviceGoodsList + '?ids=' + this.checkedArr.join(',') + '&goods_id=' + this.goods_id + '&platform=' + this.platform + '&channel_id=' + this.channel_id + '&status=' + this.status + '&sale_num=' + this.sale_num + '&start_time=' + this.start_time + '&end_time=' + this.end_time + '&is_excel=1&skey=' + this.$cookies.get('skey')
        }
    },
    methods: {
        prevClicked(){
        ...
        saveEditedGoods(){
            ...
            if(!this.goods.settlement_type){
                this.alert('结算方式必选')
                return
            }
            if(!this.goods.service_fee && this.goods.service_fee !== 0 && this.goods.service_fee !== '0'){
                this.alert('服务费必填')
                return
            }
            if(!this.goods.status && this.goods.status !== 0 && this.goods.status !== '0'){
                this.alert('推广状态必选')
                return
            }
            return request({
                url: setting.urls.serviceGoodsAdd,
                method: 'post',
        ...
        createNewChannel(){
            this.editable = true
            this.goods = {id: '', 'goods_url': '', type: 1, 'channel_id': '', 'settlement_type': 1, 'service_fee': 0, remark: '', status: ''}
        ...
        operateGoods(){
            return request({
                url: setting.urls.serviceGoodsOperate,
                method: 'post',
                data: qs.stringify({
        ...
        makeData(dt){
            ...
            this.$data['goods_add'] = dt.actions.servicegoods_add || 'F'
            this.$data['goods_import'] = dt.actions.servicegoods_import || 'F'
        ...
        loadGoodsList(pageNo){
            this.checkedArr = []
            this.allChecked = false
            this.loadTBData(setting.urls.serviceGoodsList, {
                page: pageNo || 1,
